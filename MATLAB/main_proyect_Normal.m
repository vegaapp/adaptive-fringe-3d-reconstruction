...
ppf = 10;   % Maximum fringe frequency 20  numero minim de pixeles
...
drawnow
f.Position = screens(2,:); % linea adicional para dell UPC
clim( [0, 2] )
%------------- setup adquisicion de video Camara Tijuana
...
% src.Contrast = 0;
% src.Gain = 0;
%------------- setup adquisicion de video Camara BASLER y UTB
vid = videoinput('gentl', 1, 'BayerRG8'); % 'BayerRG16'

src = getselectedsource(vid) ;
%info = propinfo(src, 'ExposureAuto')  %ConstraintValue  ExposureMode
src.BalanceWhiteAuto='Off';
ssrc.ExposureAuto = 'off';  % Continuous 'off';
src.ExposureTime = 30000 ;  %12673
src.GainAuto="off";
src.Gain=0;
src.Gamma=1;
...
% ----------- setup  procesado fases donde se Guardan
RutaImg="D:\FABIO\OPSE\MATLAB\15-may-upc\Objok24\";
filename="fp_"; %objnp24
RutaFilesfull = 'D:\FABIO\OPSE\MATLAB\15-may-upc\Objok24\fp_';
load("D:\FABIO\OPSE\MATLAB\15-may-upc\cal\cp_params.mat");

Flag_Aplica=0; % aplica mi metodo
...
for i=1:Nrejillas
    ...
    if (Flag_Aplica==1)
       ...
       disp('Mi Metodo Correccion FC');
    else
        fc_img = Proy_Base ;
        disp('Reconstruccion Normal sin Correccio FC');
    end
